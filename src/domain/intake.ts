...
export function dailyActualFor(person: Person, period: Period): DailyTargets {
  let kcal = 0;
  let p = 0;
  let c = 0;
  let f = 0;
...
export function weeklyAverageFor(person: Person, plan: Plan): DailyTargets {
  const totalDays = plan.A.days + plan.B.days;
  const a = dailyActualFor(person, plan.A);
  const b = dailyActualFor(person, plan.B);
  return {
